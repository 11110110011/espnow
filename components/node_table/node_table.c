...
esp_err_t node_table_register(const uint8_t *mac, uint8_t node_id, uint8_t caps)
{
    ...
    for (int i = 0; i < s_count; i++) {
        if (memcmp(s_nodes[i].mac, mac, 6) == 0) {
            ...
            if (!changed) { xSemaphoreGive(s_mutex); return ESP_OK; }
            goto save;
        }
    }
...
save:;
    /* Persist to NVS — make a local copy first so we can release the lock */
    node_entry_t entries[CONFIG_STORE_MAX_NODES];
    int n = s_count;
    for (int i = 0; i < n; i++) {
    ...
    xSemaphoreGive(s_mutex);
    return config_store_save_nodes(entries, n);
}
...
esp_err_t node_table_delete(uint8_t node_id)
{
    ...
    xSemaphoreGive(s_mutex);
    ESP_LOGI(TAG, "Node %d deleted (total: %d)", node_id, n);
    return config_store_save_nodes(entries, n);
}

node_record_t *node_table_find_by_id(uint8_t node_id)
{
    /* Caller must hold mutex when using the returned pointer */
    for (int i = 0; i < s_count; i++) {
...
esp_err_t node_table_update_state(uint8_t node_id, bool relay_state)
{
    ...
    if (!rec) { xSemaphoreGive(s_mutex); return ESP_ERR_NOT_FOUND; }
